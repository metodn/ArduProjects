   ...
   unsigned long currentMillis = millis(); //delay replacement
    ...
    if (currentMillis - timeOfLastEvent3 > 20){
        ...
        //        ms=1;                    //set the wariable that defines manual operation
                ...
                if (spd <=100 ) spd=100;  //lIMIT NA DOL
    ...
    if (ms==1 && currentMillis - timeOfLastEvent2 > 600   ){  //ce je branje pot vklopljeno
                ...
                spd1 = map(sensorReading, 1, 1023, 1, 50);  //  g, 1, 1023, 1, 50);  tukaj je bila liita sa nastavi hitrost of 2 do 100
                Serial.print("\r pot==>");
                Serial.print( spd1);
                Serial.print("<");
...
//
    ...
    float motorSpeed = 1000.0/((spd/60.0)/(obseg(RKROG)/PULREV));  //za izracuna cm/min v delay[ms] -> kar definira 1000



        uDTmp = micros() - uDt ;  //zracunal razliko  iz presnega delaya
        uPt = micros();    //start timer for pt----
        bool ret = one_step_function ( PinPUL, PinDIR, motorSpeed, direction );
        uPTmp = micros() - uPt;
